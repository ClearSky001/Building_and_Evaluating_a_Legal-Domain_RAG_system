"""
완전히 수정된 BM25 Stopword 리랭커 - 모든 문제 해결
"""
import sys
import os
...
# 한국어 불용어 집합
_STOP = {"의", "를", "에", "이", "가", "으로", "으로서", "및", "등", "또는", "그리고", "그", "것", "들", "는", "은", "와", "과"}


def _tokenize_wo_stop(text: str) -> list[str]:
    """불용어 제거 토큰화"""
    tokens = _tokenize_ko(text)
    return [t for t in tokens if t not in _STOP and len(t) > 1]


class BM25StopwordCompressor(BaseDocumentCompressor):
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        if not documents:
            return []
        corpus_tokens = [_tokenize_wo_stop(d.page_content) for d in documents]
        bm25 = BM25Okapi(corpus_tokens)
        scores = bm25.get_scores(_tokenize_wo_stop(query))
        ranked = sorted(zip(scores, documents), key=lambda x: x[0], reverse=True)
        return [doc for _, doc in ranked[:self.top_n]]


class LegalRAGSystemBM25Stopword:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (BM25 Stopword Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # BM25 Stopword 압축기 생성
        compressor = BM25StopwordCompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (BM25 Stopword Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemBM25Stopword()
