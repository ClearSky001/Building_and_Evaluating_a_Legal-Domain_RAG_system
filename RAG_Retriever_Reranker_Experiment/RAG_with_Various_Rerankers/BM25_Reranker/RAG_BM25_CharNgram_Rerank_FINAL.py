"""
완전히 수정된 BM25 CharNgram 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class BM25CharNgramCompressor(BaseDocumentCompressor):
    def __init__(self, top_n: int = 10, n: int = 2):
        super().__init__()
        self.top_n = top_n
        self.n = n
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        if not documents:
            return []
        corpus_tokens = [_char_ngrams(d.page_content, self.n) for d in documents]
        bm25 = BM25Okapi(corpus_tokens)
        scores = bm25.get_scores(_char_ngrams(query, self.n))
        order = np.argsort(-np.array(scores))[:self.top_n]
        return [documents[i] for i in order]


class LegalRAGSystemBM25CharNgram:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (BM25 CharNgram Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # BM25 CharNgram 압축기 생성
        compressor = BM25CharNgramCompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (BM25 CharNgram Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemBM25CharNgram()
