"""
완전히 수정된 BM25 Kiwi 리랭커 - 모든 문제 해결
"""
import sys
import os
...
try:
    ...
    print("⚠️ kiwipiepy가 설치되지 않았습니다. 기본 토크나이저를 사용합니다.")
...
class KiwiTokenizer:
    ...
    def tokenize(self, text: str) -> list[str]:
        if not self.kiwi:
            # Kiwi가 없으면 기본 토크나이저 사용
            import re
            return re.findall(r"[\w가-힣]+", text.lower())
        
        # 형태소 중 명사/형용사/동사 기본형 위주 추출
        tokens = []
        for sent in self.kiwi.split_into_sents(text):
            for t in self.kiwi.tokenize(sent.text):
                if t.tag in ("NNG","NNP","VV","VA","MAG","MAJ"):
                    lemma = t.form if t.lemma is None else t.lemma
                    tokens.append(lemma.lower())
        return tokens


class BM25KiwiCompressor(BaseDocumentCompressor):
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        if not documents:
            return []
        corpus_tokens = [self.tok.tokenize(d.page_content) for d in documents]
...
class LegalRAGSystemBM25Kiwi:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (BM25 Kiwi Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # BM25 Kiwi 압축기 생성
        compressor = BM25KiwiCompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (BM25 Kiwi Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemBM25Kiwi()
