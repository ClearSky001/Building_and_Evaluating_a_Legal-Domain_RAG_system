...
class LegalRAGSystemBM25Rerank:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (BM25 기반 Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # BM25 리랭커 생성 (새로운 인터페이스)
        self.reranker = BM25Reranker(top_n=self.rerank_top_n)
    ...
    def search(self, query: str, k: int = 10):
        ...
        try:
            ...
            # 2. BM25로 리랭킹 수행 (실제로 순서를 바꿈)
            # 초기 검색된 문서들을 BM25로 재정렬
...
def main():
    print("🚀 법률 문서 RAG 시스템 (BM25 기반 Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemBM25Rerank()
