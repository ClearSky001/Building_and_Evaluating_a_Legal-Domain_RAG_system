"""
완전히 수정된 BM25 Regex 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class BM25RegexCompressor(BaseDocumentCompressor):
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        if not documents:
            return []
        corpus_tokens = [_regex_tokens(d.page_content) for d in documents]
        bm25 = BM25Okapi(corpus_tokens)
        scores = bm25.get_scores(_regex_tokens(query))
        ranked = sorted(zip(scores, documents), key=lambda x: x[0], reverse=True)
        return [doc for _, doc in ranked[:self.top_n]]


class LegalRAGSystemBM25Regex:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (BM25 Regex Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # BM25 Regex 압축기 생성
        compressor = BM25RegexCompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (BM25 Regex Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemBM25Regex()
