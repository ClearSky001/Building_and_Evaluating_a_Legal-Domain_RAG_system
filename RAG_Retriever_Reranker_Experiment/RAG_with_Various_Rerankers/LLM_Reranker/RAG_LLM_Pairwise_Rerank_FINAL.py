"""
완전히 수정된 LLM Pairwise 리랭커 - 모든 문제 해결
"""
import sys
import os
...
PAIRWISE_PROMPT = """
다음 질문에 대해 두 문서 중 어느 것이 더 관련성이 높은지 판단해주세요.
...
더 관련성이 높은 문서를 선택해주세요. 'A' 또는 'B'로만 답하세요:
"""


class LLMPairwiseReranker(BaseDocumentCompressor):
    """LLM 기반 Pairwise 리랭킹"""
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        ...
        while pool:
            ...
            prompt = PAIRWISE_PROMPT.format(
                question=query, 
                doc_a=a.page_content[:1200], 
                doc_b=b.page_content[:1200]
            )
            
            try:
                res = self.llm.invoke(prompt).content.strip().upper()
                winner = a if res.startswith("A") else b
                winners.append(winner)
            except Exception:
                # 오류 시 첫 번째 문서 선택
                winners.append(a)
            
            if len(winners) >= self.top_n:
                break
        
        return winners[:self.top_n]


class LegalRAGSystemLLMPairwise:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (LLM Pairwise Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # LLM Pairwise 압축기 생성
        compressor = LLMPairwiseReranker(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (LLM Pairwise Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemLLMPairwise()
