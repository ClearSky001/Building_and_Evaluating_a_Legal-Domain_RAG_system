"""
완전히 수정된 CrossEncoder Electra 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class LegalRAGSystemElectraCE:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (CrossEncoder: ELECTRA-base v2) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # CrossEncoder Electra 압축기 생성
        compressor = SentenceTransformerRerank(
            model_name="cross-encoder/electra-base-korean-reranker",
...
def main():
    print("🚀 법률 문서 RAG 시스템 (CrossEncoder: ELECTRA-base v2) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemElectraCE()
