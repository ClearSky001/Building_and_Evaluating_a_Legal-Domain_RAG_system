"""
완전히 수정된 Jina v2 Base Multilingual 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class LegalRAGSystemJinaV2Base:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (Jina v2 Base Multilingual Reranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # Jina v2 Base Multilingual 압축기 생성
...
def main():
    print("🚀 법률 문서 RAG 시스템 (Jina v2 Base Multilingual Reranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemJinaV2Base()
