"""
완전히 수정된 Flashrank BGE v2m3 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class FlashrankBGEv2m3Rerank(BaseDocumentCompressor):
    """Flashrank 스타일 BGE v2m3 리랭커"""
    def __init__(self, top_n: int = 10):
        super().__init__()
        self.top_n = top_n
        try:
            # Flashrank가 없으면 일반 CrossEncoder 사용
            try:
            ...
            raise ImportError("sentence_transformers가 필요합니다")
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        ...
        if self.use_flashrank:
            ...
            return [documents[result["id"]] for result in top_results]
        else:
            # 일반 CrossEncoder 사용
...
class LegalRAGSystemFlashrankBGEv2m3:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (Flashrank BGE v2m3 Reranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # Flashrank BGE v2m3 압축기 생성
        compressor = FlashrankBGEv2m3Rerank(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (Flashrank BGE v2m3 Reranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemFlashrankBGEv2m3()
