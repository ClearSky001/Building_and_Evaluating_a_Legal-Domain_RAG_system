"""
완전히 수정된 WeightedSum BM25 Embed 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class WeightedSumBM25EmbedCompressor(BaseDocumentCompressor):
    """가중합 BM25 + Embedding 하이브리드 리랭커"""
    def __init__(self, top_n: int = 10, bm25_weight: float = 0.4, embed_weight: float = 0.6):
        super().__init__()
        self.top_n = top_n
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        ...
        # 가중합 계산
        combined_scores = self.bm25_weight * bm25_scores + self.embed_weight * embed_scores
...
class LegalRAGSystemWeightedSumBM25Embed:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (WeightedSum BM25+Embed Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # WeightedSum 압축기 생성
        compressor = WeightedSumBM25EmbedCompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (WeightedSum BM25+Embed Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemWeightedSumBM25Embed()
