"""
완전히 수정된 RRF (Reciprocal Rank Fusion) 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class RRFCompressor(BaseDocumentCompressor):
    """Reciprocal Rank Fusion 방식의 하이브리드 리랭커"""
    ...
    def compress_documents(self, documents: list[Document], query: str, callbacks=None) -> list[Document]:
        ...
        embed_ranks = embed_scores.argsort()[::-1]
        ...
        # 임베딩 순위에서 RRF 점수 계산
        for rank, doc_idx in enumerate(embed_ranks):
            rrf_scores[doc_idx] = rrf_scores.get(doc_idx, 0) + 1 / (self.k + rank + 1)
        ...
        # 상위 N개 선택
        top_indices = [doc_idx for doc_idx, score in sorted_docs[:self.top_n]]
        return [documents[i] for i in top_indices]


class LegalRAGSystemRRF:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (RRF Hybrid Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # RRF 압축기 생성
        compressor = RRFCompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (RRF Hybrid Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemRRF()
