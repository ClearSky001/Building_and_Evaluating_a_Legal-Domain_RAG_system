"""
완전히 수정된 Embedding GTE 리랭커 - 모든 문제 해결
"""
import sys
import os
...
class GTEEmbeddings(SentenceTransformerEmbeddings):
    """GTE 임베딩 모델"""
    def __init__(self, model_name: str = "thenlper/gte-multilingual-base"):
        super().__init__(model_name)


class EmbeddingCosineGTECompressor(BaseDocumentCompressor):
    """GTE 기반 임베딩 코사인 유사도 압축기"""
    def __init__(self, top_n: int = 5):
        super().__init__()
        self.top_n = top_n
        self.embed = GTEEmbeddings()
...
class LegalRAGSystemEmbeddingGTE:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (Embedding Cosine GTE Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # Embedding GTE 압축기 생성
        compressor = EmbeddingCosineGTECompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (CrossEncoder: MiniLM-L-12-v2) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemMiniLML12()
