"""
완전히 수정된 Embedding Paraphrase 리랭커 - 모든 문제 해결
"""
import sys
import os
...
from fixed_base_v2 import (
    ...
    NaiveVectorStore,
    EmbeddingCosineParaphraseCompressor,
...
class ParaEmbeddings(SentenceTransformerEmbeddings):
    """Paraphrase 임베딩 모델"""
...
class EmbeddingCosineParaphraseCompressor(BaseDocumentCompressor):
    """Paraphrase 기반 임베딩 코사인 유사도 압축기"""
    def __init__(self, top_n: int = 5):
        super().__init__()
        self.top_n = top_n
        self.embed = ParaEmbeddings()
...
class LegalRAGSystemEmbeddingParaphrase:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (Embedding Cosine Paraphrase Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # Embedding Paraphrase 압축기 생성
        self.reranker = EmbeddingCosineParaphraseCompressor(top_n=self.rerank_top_n)
