"""
완전히 수정된 Embedding Stella 리랭커 - 모든 문제 해결
"""
import sys
import os
...
from fixed_base_v2 import (
    ...
    NaiveVectorStore,
    EmbeddingCosineStellaCompressor,
...
class StellaEmbeddings(SentenceTransformerEmbeddings):
    """Stella 임베딩 모델"""
    def __init__(self, model_name: str = "InfReality/stellarmind-ko-base-v1"):
        super().__init__(model_name)


class EmbeddingCosineStellaCompressor(BaseDocumentCompressor):
    """Stella 기반 임베딩 코사인 유사도 압축기"""
    def __init__(self, top_n: int = 5):
        super().__init__()
        self.top_n = top_n
        self.embed = StellaEmbeddings()
...
class LegalRAGSystemEmbeddingStella:
    ...
    def _setup_rag_components(self):
        print("⚙️ RAG 컴포넌트 설정 중 (Embedding Cosine Stella Re-ranker) ...")
        
        # 기본 리트리버 생성
        base_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.base_k})
        
        # Embedding Stella 압축기 생성
        self.reranker = EmbeddingCosineStellaCompressor(top_n=self.rerank_top_n)
...
def main():
    print("🚀 법률 문서 RAG 시스템 (Embedding Cosine Paraphrase Re-ranker) 시작")
    print("=" * 60)
    try:
        rag_system = LegalRAGSystemEmbeddingParaphrase()
